...
import com.example.ex2.adapter.ImageAdapter;
import com.example.ex2.adapter.VideoAdapter;
import com.example.ex2.model.Image;
import com.example.ex2.model.Video;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rvPictures = findViewById(R.id.rv_images);

        if (ContextCompat.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, 1);// ask for permission
        }
        if (ContextCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, 2);// ask for permission
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 1) { // delete selected images
            if (resultCode == RESULT_OK) {
                imageAdapter.notifyDataSetChanged();
            }
        } else if (requestCode == 2) { // delete selected videos
            if (resultCode == RESULT_OK) {
